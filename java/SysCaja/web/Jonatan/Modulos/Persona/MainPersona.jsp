...
<%@page language="java" import="upeu.edu.to.PersonaTO" %>
...
<!-- class="mamani" -->
          ...
          <table border="1">
            ...
            <tbody>
               ...
               <% 
              List<PersonaTO> lista=null;
              lista=(List<PersonaTO>)request.getSession().getAttribute("listaPersona");
              int i=0;
              if(lista!=null){
              for(PersonaTO to: lista){//este for no acepta nulos
                 
                %> 
                <tr>
                  
                    <td><%=to.getIdPersona()%></td>
                    <td><%=to.getNombres()%></td>
                    <td><%=to.getApe_pat()%></td>
                    <td><%=to.getEmail()%></td>
                    <td><%=to.getCodigo()%></td>
                    <td><%=to.getFecha_nac()%></td>
                    <td><%=to.getEstado()%></td>
                    <td><%=to.getNivel()%></td>
                    <td><%=to.getEmail()%></td>
                    
                    <td>
                        <a href="../../../AlumnoControl?opt=D&idAlumno=<%=to.getIdPersona()%>">Eliminar</a>
                        <a href="#" onclick="mostrarEditForm('../../../AlumnoControl?opt=E&idAlumno=<%=to.getIdPersona()%>')">Editar</a>                        
                    </td>
                </tr>
                <% }
                }  %>
